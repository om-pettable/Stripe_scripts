...
MERGE_STRIPE_REFUND_QUERY = f"""
        MERGE INTO glue_catalog.{DB_NAME}.{TABLE_NAME} t 
            USING UpdateRefund s
            ON s.payment_intent_id = t.payment_intent_id
            WHEN MATCHED THEN UPDATE SET
                t.dispute_id = s.dispute_id,
                t.charge_id = s.charge_id,
                t.amount_refunded = s.amount_refunded,
                t.is_partially_refunded = s.is_partially_refunded,
                t.is_fully_refunded = s.is_fully_refunded,
                t.is_dispute = s.is_dispute,
                t.dispute_created_at = s.dispute_created_at,
                t.payment_refunded_at = s.payment_refunded_at,
                t.refund_data = s.refund_data,
                t.reason = s.reason,
                t.created_at = s.created_at
            WHEN NOT MATCHED THEN INSERT
            (payment_intent_id,dispute_id,charge_id,amount_refunded,is_partially_refunded,is_fully_refunded,is_dispute,dispute_created_at,payment_refunded_at,refund_data,reason,created_at)
            VALUES
            (s.payment_intent_id,s.dispute_id,s.charge_id,s.amount_refunded,s.is_partially_refunded,s.is_fully_refunded,s.is_dispute,s.dispute_created_at,s.payment_refunded_at,s.refund_data,s.reason,s.created_at)
        """
...
def read_files(spark, bucket, prefix, initial_flag,filename='', header=False, delimiter=',', escape="\"", multiline=False):
    ...
    try:
        ...
        explicit_schema = StructType([
            StructField("amount_refunded", DoubleType(), False),
            StructField("dispute_created_at", TimestampType(), False),
            StructField("payment_refunded_at", TimestampType(), False),
            StructField("is_partially_refunded", BooleanType(), False),
            StructField("is_fully_refunded", BooleanType(), False),
        ...
        df = df.withColumn("refund_data", regexp_replace(col("refund_data"), '\\"', "'"))
        df = df.withColumn("refund_data", regexp_replace(col("refund_data"), "''", '"'))
...
def apply_delta(spark, raw_bucket, file):
    ...
    try:
        ...
        if not update_df.isEmpty():
            ...
            update_df = update_df.groupBy("payment_intent_id").agg(max("payment_refunded_at").alias("payment_refunded_at")).join(update_df, ["payment_intent_id", "payment_refunded_at"], "inner")
            update_df.createOrReplaceTempView('UpdateRefund')
        ...
        if UpdateFlag:
            spark.sql(MERGE_STRIPE_REFUND_QUERY)
...
# Processing
logger.info(f"""Starting merge load for stripe_refunds tables""")
try:
    ...
    logger.error("Error while processing stripe_refund data: {}".format(str(e)))
    raise
