...
from pyspark.sql.functions import from_json, explode, regexp_replace, col, to_timestamp, translate, current_timestamp, max
...
MERGE_STRIPE_PAYMENT_QUERY = f"""
        MERGE INTO glue_catalog.{DB_NAME}.{TABLE_NAME} t 
            USING UpdatePayment s
            ...
            WHEN MATCHED THEN UPDATE SET
                ...
                t.is_dispute = s.is_dispute,
                t.payment_created_at = s.payment_created_at,
                t.payment_updated_at = s.payment_updated_at,
                t.payment_refunded_at = s.payment_refunded_at,
                t.payment_disputed_at = s.payment_disputed_at,
                t.created_at = s.created_at
            WHEN NOT MATCHED THEN INSERT
            (payment_intent_id,status,charge_id,email,phone,name,state,amount,is_refunded,is_dispute,payment_created_at,payment_updated_at,payment_refunded_at,payment_disputed_at,payment_data,created_at)
            VALUES
            (s.payment_intent_id,s.status,s.charge_id,s.email,s.phone,s.name,s.state,s.amount,s.is_refunded,s.is_dispute,s.payment_created_at,s.payment_updated_at,s.payment_refunded_at,s.payment_disputed_at,s.payment_data,s.created_at)
        """
...
def read_files(spark, bucket, prefix, initial_flag,filename='', header=False, delimiter=',', escape="\"", multiline=False):
    ...
    try:
        ...
        explicit_schema = StructType([
            ...
            StructField("payment_created_at", TimestampType(), False),
            StructField("payment_updated_at", TimestampType(), False),
            StructField("payment_refunded_at", TimestampType(), True),
            StructField("payment_disputed_at", TimestampType(), True),
            StructField("is_refunded", BooleanType(), False),
            StructField("is_dispute", BooleanType(), False)
        ])
        ...
        df = df.withColumn("payment_data", regexp_replace(col("payment_data"), '\\"', "'"))
        df = df.withColumn("payment_data", regexp_replace(col("payment_data"), "''", '"'))
...
def apply_delta(spark, raw_bucket, file):
    ...
    try:
        ...
        if not update_df.isEmpty():
            ...
            update_df = update_df.groupBy("payment_intent_id").agg(max("payment_updated_at").alias("payment_updated_at")).join(update_df, ["payment_intent_id", "payment_updated_at"], "inner")
        ...
        if UpdateFlag:
            spark.sql(MERGE_STRIPE_PAYMENT_QUERY)
...
# Processing
logger.info(f"""Starting merge load for stripe payment tables""")
try:
    ...
    logger.error("Error while processing stripe_payment data: {}".format(str(e)))
    raise
